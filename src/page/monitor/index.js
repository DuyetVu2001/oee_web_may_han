import React, { useEffect, useMemo } from 'react';
import { Route, Switch, useRouteMatch, Link, useHistory, useLocation } from 'react-router-dom';
import { HomeOutlined, UserOutlined, RetweetOutlined, SettingOutlined, UnorderedListOutlined, ClusterOutlined, ApiOutlined, DisconnectOutlined, LaptopOutlined } from '@ant-design/icons';
import { Drawer, Breadcrumb, Menu, Popover, Tabs, Input, Button, notification } from 'antd';
import styled from 'styled-components';
import { useTranslation } from "react-i18next";

// COMPONENT
import InApp from '../../com/in_app_layout';
// 
// Com
import Area from './area';
import Machine from './machine';
import Wat from './wat';
import Test from './test'
import { SRC_MACHINE, SRC_PRODUCT } from '_config/storage_key';
import TextArea from 'antd/lib/input/TextArea';
import axios from 'axios';
import { apiClient } from 'helper/request/api_client';
import { openNotificationWithIcon } from 'helper/request/notification_antd';
import { useSelector } from 'react-redux';
import { useQuery } from 'helper/hook/get_query';

// const
export const MAPPING_TITLE = {
    "View_1": "Giám sát máy",
    "View_2": "Giám sát khu vực",
    "View_3": "Giám sát điện năng",
    "View_4": "Vận hành",
}
export const LIST_TITLE = [{
    icon: ClusterOutlined,
    title: 'machine_monitor'
}, {
    icon: DisconnectOutlined,
    title: 'area_monitor'
}, {
    icon: ApiOutlined,
    title: 'power_minitor'
}, {
    icon: LaptopOutlined,
    title: 'operate'
}];
export const LIST_PAGE = ["View_1", "View_2", "View_3", "View_4"]
// 
const App = () => {
    const [pageShow, setPageShow] = React.useState(localStorage.getItem('PAGE_SHOW') || LIST_PAGE[0])
    const { userrole } = useSelector(state => state.app)
    const query = useQuery();
    const filter = React.useMemo(() => {
        return {
            machine_id: query.get('machine_id')
        }
    }, [query])

    React.useEffect(() => {
        if (pageShow) {
            localStorage.setItem('PAGE_SHOW', pageShow);
        }
        if (userrole === 5) {
            setPageShow(LIST_PAGE[3])
        }
    }, [pageShow]);
    const title = useMemo(() => {
        if (LIST_PAGE[0] === pageShow) return 'Giám sát máy và điều khiển dây truyền máy đúc nhựa'
        if (LIST_PAGE[3] === pageShow) return 'Vận hành và điều khiển dây truyền máy đúc nhựa'
    }, [pageShow])

    return (
        <InApp title={title}>
            <div>
                {LIST_PAGE[0] === pageShow && [3, 4].includes(userrole) ? <Machine filter={filter} /> : null}
                {LIST_PAGE[1] === pageShow && [3, 4].includes(userrole) ? <Area /> : null}
                {LIST_PAGE[2] === pageShow && [3, 4].includes(userrole) ? <Wat /> : null}
                {LIST_PAGE[3] === pageShow && [3, 4, 5].includes(userrole) ? <Test /> : null}
                <ChangeView pageShow={pageShow} setPageShow={setPageShow} userrole={userrole} />
            </div>
        </InApp>
    )
};

const ChangeView = ({
    pageShow = '',
    setPageShow = () => { },
    userrole
}) => {
    const [showSetting, setShowSetting] = React.useState(false);
    const { t } = useTranslation()
    const title = "home"
    return (
        <>
            <div
                style={{ position: 'fixed', bottom: 5, right: 10, display: 'flex' }}
            >
                <ButtonBottom handleClick={() => setShowSetting(true)}>
                    <RetweetOutlined />
                </ButtonBottom>
                {/*  */}
                <Drawer width={300} title={t(`${title}.title`)} visible={showSetting} onClose={() => setShowSetting(false)}>

                    {[3, 4].includes(userrole) ?
                        <ItemShow style={{ cursor: 'pointer' }} onClick={() => {
                            setPageShow(LIST_PAGE[0])
                        }} active={LIST_PAGE[0] === pageShow}>
                            <ClusterOutlined style={{ marginRight: 5 }} />{t(`${title}.${LIST_TITLE[0].title}`)}
                        </ItemShow> : null}
                    {/* {[1, 2].includes(userrole) ? <ItemShow style={{ cursor: 'pointer' }} onClick={() => {
                        setPageShow(LIST_PAGE[1])
                    }} active={LIST_PAGE[1] === pageShow}><DisconnectOutlined style={{ marginRight: 5 }} />{t(`${title}.${LIST_TITLE[1].title}`)}</ItemShow> : null}
                    {[1, 2].includes(userrole) ? <ItemShow style={{ cursor: 'pointer' }} onClick={() => {
                        setPageShow(LIST_PAGE[2])
                    }} active={LIST_PAGE[2] === pageShow}><ApiOutlined style={{ marginRight: 5 }} />{t(`${title}.${LIST_TITLE[2].title}`)}</ItemShow> : null} */}
                    {[3, 4, 5].includes(userrole) ? <ItemShow style={{ cursor: 'pointer' }} onClick={() => {
                        setPageShow(LIST_PAGE[3])
                    }} active={LIST_PAGE[3] === pageShow}><LaptopOutlined style={{ marginRight: 5 }} />{t(`${title}.${LIST_TITLE[3].title}`)}</ItemShow> : null}
                </Drawer>
            </div>
            {/* <div
                style={{ position: 'fixed', bottom: 5, right: 10, display: 'flex', marginRight: "56px" }}
            >
                <ButtonSetting handleSetting={() => {setShowSrc(true)}}>
                    <SettingOutlined />
                </ButtonSetting>
                <Drawer title ="Setting" visible={showSrc} onClose={() => setShowSrc(false)} >
                    <TextArea rows={4} value={srcIframMachine} onChange={(e) => setSrcIframMachine(e.target.value)}/>
                    <TextArea rows={4} value={srcIframProduct} onChange={(e) => setSrcIframProduct(e.target.value)}/>
                    <Button style={{position : 'absolute' , left : '34px' , bottom : "20px "}} onClick={() => {setShowSrc(false)}}>Save</Button>
                </Drawer>
            </div> */}
        </>
    )
}

const ButtonBottom = ({ handleClick = () => { }, children }) => {
    const { t } = useTranslation();
    const lang = "change_view";
    return (
        <Popover title={t(`${lang}.view`)} >
            <div
                onClick={handleClick}
                style={{
                    height: 50,
                    width: 50,
                    background: '#ddd',
                    color: '#000',
                    borderRadius: '50%',
                    opacity: '0.7',
                    display: 'flex',
                    justifyContent: 'center',
                    alignItems: 'center',
                    marginLeft: 12,
                    cursor: 'pointer',
                }}
            >
                {children}
            </div>
        </Popover>
    );
};



export default App;

const HeaderPage = ({ handleClick = () => { } }) => {
    const { t, i18n } = useTranslation();
    const history = useHistory();
    let { pathname } = useLocation();
    // let { path } = useRouteMatch();
    const list = React.useMemo(() => {
        return pathname.split('/')
    }, [pathname]);

    const listLang = React.useMemo(() => ([
        {
            title: 'Tiếng việt',
            key: 'vn',
        },
        {
            title: 'English',
            key: 'en',
        },
        {
            title: '日本',
            key: 'ja',
        },
        {
            title: '한국인',
            key: 'ko',
        },
    ]), []);


    const _handleChangeLang = React.useCallback((val) => {
        if (val == "vn") {
            i18n.changeLanguage(val);
            localStorage.setItem("lang", val);
            apiClient.post("/user/language", {
                language: "vi"
            })
                .then((data) => {
                    console.log("1111", data);
                    openNotificationWithIcon("success", data.data.msg)
                })
                .finally((data) => {
                    window.location.reload()
                    openNotificationWithIcon("success", data.data.msg)
                })
        }
        else {
            i18n.changeLanguage(val);
            localStorage.setItem("lang", val);
            apiClient.post("/user/language", {
                language: val
            })
                .finally(() => {
                    window.location.reload()
                })
        }
    }, [])

    const content = (
        <ListSelect>
            {listLang.map(({ key, title }) => <div
                key={key}
                style={i18n.language === key ? { background: '#ddd' } : {}}
                onClick={() => _handleChangeLang(key)}>{title}</div>)}
        </ListSelect>
    )
    return (
        <div style={{
            borderBottom: '1px solid #eee', padding: '4px 0px',
            display: 'flex', justifyContent: 'space-between', alignItems: 'center'
        }}>
            <Breadcrumb>
                {list.map((pathName, index) => {
                    if (!index) return (
                        <Breadcrumb.Item key="home" onClick={window.innerWidth > 416 ? () => {
                            history.push('/')
                        } : () => {
                            history.push('/')
                            handleClick()
                        }} >
                            {window.innerWidth > 416 ? <HomeOutlined style={{ cursor: 'pointer' }} /> : <UnorderedListOutlined style={{ cursor: 'pointer' }} />}
                        </Breadcrumb.Item>
                    )
                    if (!pathName) return null;
                    return (
                        <Breadcrumb.Item key={pathName} onClick={() => index !== list.length - 1 && history.push(`/${pathName}/`)}>
                            <span style={{ textTransform: 'capitalize' }}>{pathName}</span>
                        </Breadcrumb.Item>
                    )
                })}
            </Breadcrumb>
            <div style={{ display: 'flex' }}>
                <Popover style={{ padding: 0 }} placement="topLeft" title={null} content={content}
                //  trigger="click"
                >
                    <img src="data:image/jpeg;base64," style={{ height: 20, width: 20, marginRight: 10 }} />
                </Popover>

                <UserOutlined onClick={() => history.push('/profile')} />
            </div>

        </div>
    );
};

const ListSelect = styled.div`
    div {
        padding: 5px 10px;
        cursor : pointer
    }
    div:hover {
        background: #ddd
    }
    
`

const ItemShow = styled.div`
	padding: 10px 20px;
	font-size: 1.3em;
	background: ${({ active }) => active ? "#ddd" : '#fff'}
`;