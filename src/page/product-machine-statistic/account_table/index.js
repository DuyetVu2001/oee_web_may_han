...
import { _handleDownLoadFile } from "com/BtnDownload";
...
const TableFunction = ({ hideMachine }) => {
    ...
    const title = "statistics_machine";
    const change_view = "change_view";
    return (
        <div style={{}}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>

                <span style={{ fontSize: 16, fontWeight: 600 }}>{t(`${title}.statistics_machine`)}</span>
                <Extra
        ...
        <div style={{ display: 'flex', alignItems: 'center', paddingRight: 7, justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', flex: 1 }}>
                <div style={{ display: 'flex' }}>
                    ...
                    <Button loading={loading} onClick={() => _onDownload()} className="ro-custom" type="text" icon={<DownloadOutlined />} >{t(`${title}.down`)}</Button>
                    <Button loading={loading} onClick={() => _onReload()} className="ro-custom" type="text" icon={<ReloadOutlined />} >{t(`${title}.reset`)}</Button>
                    {/* <Button loading={loading} onClick={_onClickAdd} className="ro-custom" type="text" icon={<PlusOutlined />} >Thêm mới</Button> */}
                    <Button loading={loading} onClick={_onFilter} className="ro-custom" type="text" icon={<FilterOutlined />} >{t(`${title}.filter`)}</Button>
