import { Button, DatePicker, Drawer, Select, Skeleton, Form, Input, message, Popover, Breadcrumb, Tabs, Tooltip, Row, Col } from 'antd';
...
const App = ({ input, showMachine, Com }) => {
    ...
    React.useEffect(() => {
        if (input === "statistic") {
            setTimestamp({
                from: moment().valueOf() - 24 * 60 * 60 * 1000,
                to: moment().valueOf()
            })
        }
        if (input === "statistic_line") {
            setTimestamp({
                from: moment().valueOf() - 7 * 24 * 60 * 60 * 1000,
                to: moment().valueOf()
            })
        }
        if (input === "power_statistic") {
            setTimestamp({
                from: moment().valueOf() - 30 * 24 * 60 * 60 * 1000,
                to: moment().valueOf()
            })
        }
        setSrcRealTime(srcRealtime)
        requestInitData();
    }, []);
