import { EditOutlined, HomeOutlined, UserOutlined } from '@ant-design/icons';
import { Breadcrumb, Button, Popover, Select } from 'antd';
import { CardCustom } from 'page/account/account_table/helper/styled_component';
import React, { useEffect, useState } from 'react';
import InApp from '../../com/in_app_layout'
import { get } from "lodash";
import { useTranslation, initReactI18next } from "react-i18next";
import ModalEditProfile from './Com/ModalEditProfile';
import ChangePasswordModal from './Com/ChangePasswordModal';
import { getProfile } from './service';
import { useDispatch } from 'react-redux';
import { requestLogout } from 'app_state/login';
import { images } from 'helper/static/images';
import { setProfile } from './service';
import { openNotificationWithIcon } from 'helper/request/notification_antd';
import { handleErr } from 'helper/request/handle_err_request';
import { useHistory, useLocation } from 'react-router-dom/cjs/react-router-dom.min';
import styled from 'styled-components';
import axios from 'axios';
import { apiClient } from 'helper/request/api_client';


const ProfilePage = () => {
    const [modalProfileData, setModalProfileData] = useState(null);
    const [modalChangePwdData, setModalChangePwdData] = useState(null);
    const [line, setLine] = useState([])
    const [dataProfile , setDataProfile] = useState([])
    const { t, i18n } = useTranslation();

    const lang = "profile"

    const dispatch = useDispatch()

    const _requestDataTable = async() => {
        try {
            const {data} = await getProfile({});
            setDataProfile(data)
        } catch (error) {
            
        }
    }
    useEffect(() => {
        _requestDataTable()
    },[])

    const langProfile = "profile";
    const handleEditProfile = () => {
        // requestFetchProfile();
        setModalProfileData(null);
    };

    const handleChangePwd = (val) => {
        console.log('asdfasdf', val)
        setProfile({
            id: dataProfile.id,
            old_password: val.old_password,
            new_password: val.new_password
        })
        .then(() => {
            openNotificationWithIcon('success', 'update success')
            setModalChangePwdData(null);
        })
        .catch((err) => {
            handleErr(err)
        })
    };

    const handleUpdateOke = () => {
        // requestFetchProfile();
        // props.requestProfile();
    }

    const _handleChangeLang = (val) => {
        if (val == "vn") {
            i18n.changeLanguage(val);
            localStorage.setItem("lang", val);
            apiClient.post("/user/language", {
                language: "vi"
            })
            .finally(() => {
                window.location.reload()
            })
        }
        else {
            i18n.changeLanguage(val);
            localStorage.setItem("lang", val);
            apiClient.post("/user/language", {
                language: val
            })
            .finally(() => {
                window.location.reload()
            })
        }
    }

    return (
        <InApp BreadCum={HeaderPage}>
            <CardCustom
                title={null}
                extra={
                    <div>
                        <div style={{ position: "fixed", bottom: 20, right: 20 }}>
                            <Select value={i18n.language} defaultValue="vn" onChange={_handleChangeLang}>
                                <Select.Option value="en">English</Select.Option>
                                <Select.Option value="vn">Việt Nam</Select.Option>
                                <Select.Option value="ja">Japan</Select.Option>
                                <Select.Option value="ko">korean</Select.Option>
                            </Select>
                        </div>
                        {/* <Button
                            onClick={() => dispatch(requestLogout())}
                            style={{ borderRadius: 6 }}
                            type="primary"
                            danger
                        >
                            {t(`${langProfile}.log_out`)}
                        </Button> */}
                    </div>
                }
            >
                <div style={{ display: 'flex', padding: '0px 50px', }}>
                    <div style={{ height: '30vh', width: '30vh', borderRadius: '30vh', border: '1px solid #dfdfdf', position: 'relative', marginTop: 40 }}>
                        <img src={images.avatar} style={{ height: '100%', width: '100%' }} />

                        <div
                            onClick={() => setModalProfileData({ data: dataProfile, ...dataProfile })}
                            style={{ position: 'absolute', height: 40, width: 40, display: 'flex', borderRadius: 20, justifyContent: 'center', alignItems: 'center', top: 0, right: 0, backgroundColor: '#fff', boxShadow: '3px 3px 1.5px #B0B0B0' }}>
                            <EditOutlined style={{ fontSize: 20, }} />
                        </div>
                    </div>
                    <div
                        style={{
                            padding: "20px",
                            justifyContent: "center",
                            display: "flex",
                            flexDirection: "column",
                        }}
                    >
                        <ul className="a">
                            <li>{`${t(`${lang}.user_name`)}: ${dataProfile.username}`}</li>
                            <li>{`${t(`${lang}.name`)}: ${dataProfile.name}`}</li>
                            <li>{`${t(`${lang}.phone`)}: ${dataProfile.phone}`}</li>
                            <li>{`${t(`${lang}.email`)}: ${dataProfile.email}`}</li>
                            {/* <li>{`Vai trò: ${dataProfile.role_id}`}</li>
                            <li>{`Level : ${dataProfile.level}`}</li> */}
                            {/* <li>Máy ẩn:</li> */}
                        </ul>

                        {/* <div>
                            <Button onClick={() => setModalChangePwdData({ data: 1 })}>
                               {t(`${lang}.change_pw`)}
                            </Button>
                        </div> */}

                        <ModalEditProfile
                            visible={modalProfileData}
                            onCancel={() => setModalProfileData(null)}
                            handleAction={handleEditProfile}
                            line={line}
                            handleOk={handleUpdateOke}
                        />

                        <ChangePasswordModal
                            visible={modalChangePwdData}
                            onCancel={() => setModalChangePwdData(null)}
                            handleAction={handleChangePwd}
                            // requestSignOut={requestSignOut}
                        />
                    </div>
                </div>
            </CardCustom>
        </InApp>
    );
};

const HeaderPage = ({handleClick = () => {}}) => {
    const { t, i18n } = useTranslation();
    const history = useHistory();
    let { pathname } = useLocation();
    // let { path } = useRouteMatch();
    const list = React.useMemo(() => {
        return pathname.split('/')
    }, [pathname]);

    const listLang = React.useMemo(() => ([
        {
            title: 'Tiếng việt',
            key: 'vn',
        },
        {
            title: 'English',
            key: 'en',
        },
        {
            title: '日本',
            key: 'ja',
        },
        {
            title: '한국인',
            key: 'ko',
        },
    ]), []);


    const _handleChangeLang = React.useCallback((val) => {
        i18n.changeLanguage(val);
        localStorage.setItem("lang", val);
    }, [])

    const content = (
        <ListSelect>
            {listLang.map(({ key, title }) => <div
                key={key}
                style={i18n.language === key ? { background: '#ddd' } : {}}
                onClick={() => _handleChangeLang(key)}>{title}</div>)}
        </ListSelect>
    )
    return (
        <div style={{
            borderBottom: '1px solid #eee', padding: '4px 0px',
            display: 'flex', justifyContent: 'space-between', alignItems: 'center'
        }}>
            <Breadcrumb>
                {list.map((pathName, index) => {
                    if (!index) return (
                        <Breadcrumb.Item key="home" onClick={window.innerWidth >416 ?() => {
                            history.push('/')
                        } : () => {
                            history.push('/')
                            handleClick()
                        }} >
                            <HomeOutlined style={{cursor : 'pointer'}}/>
                        </Breadcrumb.Item>
                    )
                    if (!pathName) return null;
                    return (
                        <Breadcrumb.Item key={pathName} onClick={() => index !== list.length - 1 && history.push(`/${pathName}/`)}>
                            <span style={{ textTransform: 'capitalize' }}>{pathName}</span>
                        </Breadcrumb.Item>
                    )
                })}
            </Breadcrumb>
            <div style={{ display: 'flex' }}>
                <Popover style={{ padding: 0 }} placement="topLeft" title={null} content={content}
                //  trigger="click"
                >
                    <img src="data:image/jpeg;base64," style={{ height: 20, width: 20, marginRight: 10 }}  />
                </Popover>
            </div>

        </div>
    );
};
const ListSelect = styled.div`
    div {
        padding: 5px 10px;
    }
    div:hover {
        background: #ddd
    }
`
export default ProfilePage;